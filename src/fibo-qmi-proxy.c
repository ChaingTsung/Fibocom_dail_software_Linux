...
static int pthread_cond_timeout_np(pthread_cond_t *cond, pthread_mutex_t *mutex,
				   unsigned msecs)
{
    if (msecs != 0) {
	struct timespec ts;
...
static int create_local_server(const char *name)
{
    ...
    SYSCHECK(setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &reuse_addr,
			sizeof(reuse_addr)));
...
static void cleanup_qmi_connection(int clientfd)
{
    ...
    {
	QMI_PROXY_CONNECTION *qmi_con =
	    qnode_to_item(con_node, QMI_PROXY_CONNECTION, qnode);

	if (qmi_con->ClientFd == clientfd) {
	    while (!qlist_empty(&qmi_con->client_qnode)) {
		QMI_PROXY_CLINET *qmi_client =
		    qnode_to_item(qlist_head(&qmi_con->client_qnode),
				  QMI_PROXY_CLINET, qnode);

		dprintf("xxx ClientFd=%d QMIType=%d ClientId=%d\n",
			qmi_con->ClientFd, qmi_client->QMIType,
			qmi_client->ClientId);
		...
		QMI_PROXY_MSG *qmi_msg =
		    qnode_to_item(qmi_node, QMI_PROXY_MSG, qnode);
...
static void get_client_id(QMI_PROXY_CONNECTION *qmi_con,
			  PQMICTL_GET_CLIENT_ID_RESP_MSG pClient)
{
    if (pClient->QMIResult == 0 && pClient->QMIError == 0) {
	QMI_PROXY_CLINET *qmi_client =
	    (QMI_PROXY_CLINET *)malloc(sizeof(QMI_PROXY_CLINET));
	...
	dprintf("+++ ClientFd=%d QMIType=%d ClientId=%d\n", qmi_con->ClientFd,
		qmi_client->QMIType, qmi_client->ClientId);
	qlist_add_tail(&qmi_con->client_qnode, &qmi_client->qnode);
    }
}

static void release_client_id(QMI_PROXY_CONNECTION *qmi_con,
			      PQMICTL_RELEASE_CLIENT_ID_RESP_MSG pClient)
{
	...
	{
	    QMI_PROXY_CLINET *qmi_client =
		qnode_to_item(client_node, QMI_PROXY_CLINET, qnode);

	    if (pClient->QMIType == qmi_client->QMIType &&
		pClient->ClientId == qmi_client->ClientId) {
		dprintf("--- ClientFd=%d QMIType=%d ClientId=%d\n",
			qmi_con->ClientFd, qmi_client->QMIType,
			qmi_client->ClientId);
		qlist_remove(&qmi_client->qnode);
...
static int send_qmi_to_cdc_wdm(PQCQMIMSG pQMI)
{
	...
	if (verbose_debug) {
	    ...
	    for (i = 0; i < 16; i++) printf("%02x ", ((uint8_t *)pQMI)[i]);
	    printf("\n");
	}
    }
...
static void recv_qmi(PQCQMIMSG pQMI, unsigned size)
{
	...
	if (s_pCtlReq != NULL) {
		...
		s_pCtlReq->CTLMsg.QMICTLMsgHdrRsp.TransactionId ==
		    pQMI->CTLMsg.QMICTLMsgHdrRsp.TransactionId) {
		s_pCtlRsq = malloc(size);
		memcpy(s_pCtlRsq, pQMI, size);
		pthread_cond_signal(&s_ctlcond);
	    } else if (pQMI->QMIHdr.QMIType != QMUX_TYPE_CTL &&
		       s_pCtlReq->MUXMsg.QMUXMsgHdr.TransactionId ==
			   pQMI->MUXMsg.QMUXMsgHdr.TransactionId) {
		s_pCtlRsq = malloc(size);
		memcpy(s_pCtlRsq, pQMI, size);
		pthread_cond_signal(&s_ctlcond);
	    }
	...
	if (pQMI->CTLMsg.QMICTLMsgHdr.CtlFlags == QMICTL_CTL_FLAG_RSP) {
		...
		QMI_PROXY_MSG *qmi_msg = qnode_to_item(
		    qlist_head(&qmi_proxy_ctl_msg), QMI_PROXY_MSG, qnode);
		...
		qlist_for_each(con_node, &qmi_proxy_connection)
		{
			...
			if (pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType ==
			    QMICTL_GET_CLIENT_ID_RESP)
			    get_client_id(qmi_con,
					  &pQMI->CTLMsg.GetClientIdRsp);
			else if (pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType ==
				 QMICTL_RELEASE_CLIENT_ID_RESP)
			    release_client_id(qmi_con,
					      &pQMI->CTLMsg.ReleaseClientIdRsp);
			else {
			}
		    }
		}
	...
	qlist_for_each(con_node, &qmi_proxy_connection)
	{
		...
		if (pQMI->QMIHdr.QMIType == qmi_client->QMIType) {
			...
			pQMI->QMIHdr.ClientId == qmi_client->ClientId) {
...
static int send_qmi_timeout(PQCQMIMSG pRequest, PQCQMIMSG *ppResponse,
			    unsigned mseconds)
{
    int ret;
...
static PQCQMUX_TLV qmi_find_tlv(PQCQMIMSG pQMI, uint8_t TLVType)
{
	...
	PQCQMUX_TLV pTLV =
	    (PQCQMUX_TLV)(&pQMI->MUXMsg.QMUXMsgHdr.payload[Length]);
...
static int qmi_proxy_init(void)
{
	...
	pQMI->QMIHdr.Length = pQMI->CTLMsg.QMICTLMsgHdr.Length +
			      sizeof(QCQMI_HDR) + sizeof(QCQMICTL_MSG_HDR) - 1;
    ...
    pQMI->QMIHdr.Length = pQMI->CTLMsg.QMICTLMsgHdr.Length + sizeof(QCQMI_HDR) +
			  sizeof(QCQMICTL_MSG_HDR) - 1;
	...
	if (pRsp->CTLMsg.QMICTLMsgHdrRsp.QMUXResult ||
	    pRsp->CTLMsg.QMICTLMsgHdrRsp.QMUXError) {
	    dprintf("QMICTL_GET_VERSION_REQ QMUXResult=%d, QMUXError=%d\n",
		    pRsp->CTLMsg.QMICTLMsgHdrRsp.QMUXResult,
		    pRsp->CTLMsg.QMICTLMsgHdrRsp.QMUXError);
	    goto qmi_proxy_init_fail;
	}

	for (NumElements = 0;
	     NumElements < pRsp->CTLMsg.GetVersionRsp.NumElements;
	     NumElements++) {
	    if (verbose_debug)
		dprintf("QMUXType = %02x Version = %d.%d\n",
			pRsp->CTLMsg.GetVersionRsp.TypeVersion[NumElements]
			    .QMUXType,
			pRsp->CTLMsg.GetVersionRsp.TypeVersion[NumElements]
			    .MajorVersion,
			pRsp->CTLMsg.GetVersionRsp.TypeVersion[NumElements]
    ...
    rx_urb_size =
	32 * 1024;  // must same as rx_urb_size defined in GobiNet&qmi_wwan
		    // driver //SDX24&SDX55 support 32KB
    ep_type = 0x02;
    iface_id = 0x04;
    ...
    pQMI->MUXMsg.SetDataFormatReq.Length =
	sizeof(QMIWDS_ADMIN_SET_DATA_FORMAT_REQ_MSG) - sizeof(QCQMUX_MSG_HDR);

    // Indicates whether the Quality of Service(QOS) data format is used by the
    // client.
    pQMI->MUXMsg.SetDataFormatReq.QosDataFormatTlv.TLVType = 0x10;
    pQMI->MUXMsg.SetDataFormatReq.QosDataFormatTlv.TLVLength =
	cpu_to_le16(0x0001);
    pQMI->MUXMsg.SetDataFormatReq.QosDataFormatTlv.QOSSetting =
	0; /* no-QOS header */
    // Underlying Link Layer Protocol
    pQMI->MUXMsg.SetDataFormatReq.UnderlyingLinkLayerProtocolTlv.TLVType = 0x11;
    pQMI->MUXMsg.SetDataFormatReq.UnderlyingLinkLayerProtocolTlv.TLVLength =
	cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.UnderlyingLinkLayerProtocolTlv.Value =
	cpu_to_le32(0x02); /* Set IP mode */
    // Uplink (UL) data aggregation protocol to be used for uplink data
    // transfer.
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationProtocolTlv.TLVType =
	0x12;
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationProtocolTlv.TLVLength =
	cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationProtocolTlv.Value =
	cpu_to_le32(0x05);  // UL QMAP is enabled
    // Downlink (DL) data aggregation protocol to be used for downlink data
    // transfer
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationProtocolTlv.TLVType =
	0x13;
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationProtocolTlv.TLVLength =
	cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationProtocolTlv.Value =
	cpu_to_le32(0x05);  // UL QMAP is enabled
    // Maximum number of datagrams in a single aggregated packet on downlink
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationMaxDatagramsTlv
	.TLVType = 0x15;
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationMaxDatagramsTlv
	.TLVLength = cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationMaxDatagramsTlv.Value =
	cpu_to_le32((rx_urb_size > 2048) ? (rx_urb_size / 1024) : 1);
    // Maximum size in bytes of a single aggregated packet allowed on downlink
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationMaxSizeTlv.TLVType =
	0x16;
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationMaxSizeTlv.TLVLength =
	cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationMaxSizeTlv.Value =
	cpu_to_le32(rx_urb_size);
    ...
    pQMI->MUXMsg.SetDataFormatReq.epTlv.ep_type =
	cpu_to_le32(ep_type);  // DATA_EP_TYPE_BAM_DMUX
    pQMI->MUXMsg.SetDataFormatReq.epTlv.iface_id = cpu_to_le32(iface_id);

    pQMI->QMIHdr.Length = pQMI->MUXMsg.QMUXMsgHdr.Length + sizeof(QCQMI_HDR) +
			  sizeof(QCQMUX_MSG_HDR) - 1;
	...
	if (pRsp->MUXMsg.QMUXMsgHdrResp.QMUXResult ||
	    pRsp->MUXMsg.QMUXMsgHdrResp.QMUXError) {
	    dprintf(
		"QMIWDS_ADMIN_SET_DATA_FORMAT_REQ QMUXResult=%d, "
		"QMUXError=%d\n",
		pRsp->MUXMsg.QMUXMsgHdrResp.QMUXResult,
		pRsp->MUXMsg.QMUXMsgHdrResp.QMUXError);
	    goto qmi_proxy_init_fail;
	}
...
static void qmi_start_server(void)
{
	...
	qmi_proxy_server_fd = create_local_server("fibocom-qmi-proxy");
    printf("%s: qmi_proxy_server_fd = %d\n", __func__, qmi_proxy_server_fd);
    if (qmi_proxy_server_fd == -1) {
	dprintf("%s Failed to create %s, errno: %d (%s)\n", __func__,
		//"quectel-qmi-proxy", errno, strerror(errno));
		"fibocom-qmi-proxy", errno, strerror(errno));
    }
}
	...
	for (ne = 0; ne < nevents; ne++) {
		...
		if (nreads <= 0) {
		    dprintf("%s read=%d errno: %d (%s)\n", __func__,
			    (int)nreads, errno, strerror(errno));
		    goto qmi_proxy_loop_exit;
		}
		...
		if (nreads <= 0) {
		    dprintf("%s read=%d errno: %d (%s)", __func__, (int)nreads,
...
qmi_proxy_loop_exit:
	...
	QMI_PROXY_CONNECTION *qmi_con = qnode_to_item(
	    qlist_head(&qmi_proxy_connection), QMI_PROXY_CONNECTION, qnode);
...
static void qmidevice_detect(char **device_name)
{
	...
	while ((ent = readdir(pDir)) != NULL) {
		...
		strncasecmp(idVendor, "2c7c", 4))
		continue;

	    dprintf("Find %s/%s idVendor=%s idProduct=%s\n", dir, ent->d_name,
		    idVendor, idProduct);
	    snprintf(pl->subdir, sizeof(pl->subdir), "%s/%s:1.4/usbmisc", dir,
		     ent->d_name);
	    if (access(pl->subdir, R_OK)) {
		snprintf(pl->subdir, sizeof(pl->subdir), "%s/%s:1.4/usb", dir,
			 ent->d_name);
		if (access(pl->subdir, R_OK)) {
...
{
    dprintf(
	" -d <device_name>                      A valid qmi device\n"
	"                                       default /dev/cdc-wdm0, but "
	"cdc-wdm0 may be invalid\n"
...
int main(int argc, char *argv[])
{
	...
	while (qmi_proxy_init() != 0) {
		...
		dprintf(
		    "has failed too much times, restart the modem and have "
		    "a try...\n");
		break;
	    }
